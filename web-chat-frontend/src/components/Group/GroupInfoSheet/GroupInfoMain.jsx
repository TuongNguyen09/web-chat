...
const GroupInfoMain = ({
  ...
  return (
    ...
    >
      <div className="w-full md:w-[400px] lg:w-[450px] h-full bg-white shadow-2xl flex flex-col animate-slide-in-right">
        {/* Header */}
        <div className="flex items-center px-4 py-3 bg-white border-b border-gray-100 shrink-0">
          ...
          >
            <AiOutlineClose className="text-gray-600 text-xl" />
          </button>
        ...
        <div className="flex-1 overflow-y-auto bg-gray-50 custom-scrollbar">
          {/* Avatar & Name Section */}
          <div className="bg-white px-6 py-8 mb-3 shadow-sm flex flex-col items-center">
            <div className="relative group mb-4">
              <div className="w-32 h-32 md:w-40 md:h-40 rounded-full overflow-hidden shadow-md border-4 border-white">
                {isUploadingAvatar ? (
            ...
            <div className="w-full flex flex-col items-center gap-2">
              <div className="flex items-center gap-2 max-w-full">
                {isCurrentUserAdmin ? (
                  <div className="flex items-center border-b border-transparent hover:border-gray-300 focus-within:border-[#00a884] transition-colors pb-1">
                    <input
                      className="text-xl md:text-2xl font-normal text-gray-900 text-center bg-transparent outline-none min-w-[100px]"
                      value={nameDraft}
                      onChange={(e) => setNameDraft(e.target.value)}
                      onBlur={() => onRequestNameChange?.(nameDraft)}
                      onKeyDown={(e) => e.key === "Enter" && e.target.blur()}
                    />
                    <BsPencil className="text-gray-400 ml-2 cursor-pointer" />
                  </div>
                ) : (
                  <h2 className="text-xl md:text-2xl font-normal text-gray-900 text-center">
                    {chat.chatName}
                  </h2>
                )}
              </div>
...
const Details = () => (
  <div className="bg-white shadow-sm px-2 py-2">
    <div className="divide-y">
      {["Cài đặt nhóm", "Quyền của quản trị viên"].map((text) => (
        <button
          key={text}
          className="flex items-center justify-between w-full px-4 py-4 hover:bg-gray-50 transition-colors rounded-lg"
        >
    ...
    <div className="mt-2 px-4 py-2">
      <p className="text-xs text-gray-400 uppercase font-bold mb-2">Tùy chọn khác</p>
      <div className="grid grid-cols-1 gap-1">
        {["Tìm kiếm tin nhắn", "File phương tiện, link và tài liệu"].map((item) => (
          <button
            key={item}
            className="text-left py-3 px-2 text-gray-700 hover:text-[#00a884] transition-colors"
          >
            {item}
          </button>
        ))}
